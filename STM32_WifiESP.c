...
ESP01_Status_t esp01_init(UART_HandleTypeDef *huart_esp, UART_HandleTypeDef *huart_debug, uint8_t *dma_rx_buf, uint16_t dma_buf_size)
{
    ...
    HAL_Delay(100); // Petit délai pour laisser l'ESP01 démarrer

    ESP01_Status_t status = esp01_test_at();
    if (status != ESP01_OK)
    {
        ESP01_LOG_ERROR("INIT", "ESP01 non détecté !");
        return ESP01_NOT_DETECTED;
    }
    return ESP01_OK;
}
...
ESP01_Status_t esp01_flush_rx_buffer(uint32_t timeout_ms)
{
    ...
    while ((HAL_GetTick() - start) < timeout_ms) // Boucle jusqu'à expiration du timeout
    {
        ...
        if (len == 0)                                       // Si rien à lire, attend un peu
            HAL_Delay(1);                                   // Petite pause pour éviter de surcharger le CPU
    }
...
{
    VALIDATE_PARAM(version_buf && buf_size > 0, ESP01_INVALID_PARAM); // Vérifie les paramètres d'entrée

    char resp[ESP01_MAX_RESP_BUF];                                                                           // Buffer pour la réponse complète
    ESP01_Status_t st = esp01_send_raw_command_dma("AT+GMR", resp, sizeof(resp), "OK", ESP01_TIMEOUT_SHORT); // Envoie "AT+GMR" et attend "OK"                                                    // Log la réponse reçue

    if (st != ESP01_OK) // Si la commande AT+GMR a échoué
    {
        ESP01_LOG_ERROR("GMR", "Erreur : %s", esp01_get_error_string(st)); // Log l'erreur
        return ESP01_FAIL;                                                 // Retourne FAIL
    }

    size_t resp_len = strlen(resp);                                  // Calcule la longueur de la réponse
    if (esp01_check_buffer_size(resp_len, buf_size - 1) != ESP01_OK) // Vérifie que le buffer de sortie est assez grand
        return ESP01_BUFFER_OVERFLOW;                                // Retourne overflow si trop petit
    ...
    // Tu peux copier version_only dans version_buf si tu veux retourner la version parsée
    strncpy(version_buf, version_only, buf_size - 1);
    version_buf[buf_size - 1] = '\0';
    ESP01_LOG_INFO("GMR", "Version du firmware :\n%s", version_buf);
...
ESP01_Status_t esp01_sleep_mode_to_string(int mode, char *out, size_t out_size)
{
    ...
    {
    case 0:
        desc = "Pas de sommeil (no sleep, 0)"; // Mode 0
        break;
    case 1:
        desc = "Sommeil léger (light sleep, 1)"; // Mode 1
        break;
    case 2:
        desc = "Sommeil modem (modem sleep, 2)"; // Mode 2
        break;
    }
...
ESP01_Status_t esp01_syslog_to_string(int syslog, char *out, size_t out_size)
{
    ...
    {
    case 0:
        desc = "Désactivé (0)"; // Niveau 0
        break;
    case 1:
        desc = "Erreur (1)"; // Niveau 1
        break;
    case 2:
        desc = "WARNING (2)"; // Niveau 2
        break;
    case 3:
        desc = "Info (3)"; // Niveau 3
        break;
    case 4:
        desc = "Debug (4)"; // Niveau 4
        break;
    }
    snprintf(out, out_size, "%s", desc);                // Copie la description dans le buffer
 ...
 * @param  out_size Taille du buffer de sortie (doit être grand, ex: ESP01_LARGE_RESP_BUF).
...
{
    if (needed > available)
        ESP01_RETURN_ERROR("BUFFER", ESP01_BUFFER_OVERFLOW);
    return ESP01_OK;
}
...
static ESP01_Status_t esp01_interactive_at_console(char *out_buf, size_t out_buf_size)
{
    ...
    if (!esp_console_cmd_ready || esp_console_cmd_idx == 0) // Vérifie qu'une commande est prête à être envoyée
        return ESP01_FAIL;                                  // Retourne FAIL si rien à envoyer

    uint32_t timeout = ESP01_TIMEOUT_SHORT; // Timeout par défaut

    const char *cmds_long[] = {// Liste des commandes nécessitant un timeout long
                               "AT+CWJAP", "AT+CWLAP", "AT+CIPSTART", "AT+CIPSEND", "AT+MQTTCONN", "AT+HTTPCLIENT", "AT+RESTORE", "AT+UPDATE", "AT+CMD?", NULL};

    for (int i = 0; cmds_long[i]; ++i) // Parcourt la liste des commandes longues
    {
        if (strncmp((char *)esp_console_cmd_buf, cmds_long[i], strlen(cmds_long[i])) == 0) // Compare le début de la commande
        {
            timeout = ESP01_TIMEOUT_LONG; // Timeout long pour les commandes longues
            break;
        }
    }
...
{
    if (huart == g_debug_uart) // Vérifie que c'est l'UART debug
    {
 ...
 */
ESP01_Status_t esp01_log_pattern_not_found(const char *context, const char *pattern)
{
    if (pattern)
    {
        ESP01_LOG_ERROR("PATTERN", "[%s] motif '%s' non trouvé", context, pattern);
    }
    else
    {
        ESP01_LOG_ERROR("PATTERN", "[%s] motif non trouvé", context);
    }
    return ESP01_FAIL; // Retourne FAIL
}
