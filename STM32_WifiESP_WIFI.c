...
#include <stdlib.h>             // Pour atoi(), etc.
...
ESP01_Status_t esp01_get_connection_status(void)
{
    ...
    if (st != ESP01_OK)
    {
        ESP01_LOG_ERROR("STATUS", "Erreur lors de la vérification du statut: %s", esp01_get_error_string(st));
        return st;
    }
    ...
    {
        ESP01_LOG_INFO("STATUS", "WiFi connecté");
        return ESP01_OK;
    }
 ...
 *              - 1: Station (STA)
 *              - 2: Access Point (AP)
 *              - 3: Station + Access Point (STA+AP)
 * @retval ESP01_Status_t ESP01_OK en cas de succès ou code d'erreur
 * @note   Cette fonction envoie la commande AT+CWMODE?
 * @details La fonction envoie AT+CWMODE? et parse la réponse pour
 *          extraire le mode. Le format de réponse attendu est:
 *          +CWMODE:X où X est un nombre entier représentant le mode.
 */
ESP01_Status_t esp01_get_wifi_mode(uint8_t *mode)
{
    ...
    // Utilisation d'une variable temporaire int32_t pour le parsing
    int32_t mode_tmp = 0;
    ...
    // Conversion sécurisée vers uint8_t
    *mode = (uint8_t)mode_tmp;
    ESP01_LOG_INFO("CWMODE", "Mode WiFi actuel: %d (%s)", *mode, esp01_wifi_mode_to_string(*mode));
    return ESP01_OK;
}

/**
 * @brief  Définit le mode WiFi.
 * @param  mode Mode WiFi à configurer.
...
ESP01_Status_t esp01_set_wifi_mode(int mode)
{
    ...
    ESP01_LOG_INFO("CWMODE", "Mode WiFi configuré à %d (%s)", mode, esp01_wifi_mode_to_string(mode));
    return ESP01_OK;
}
...
ESP01_Status_t esp01_get_tcp_status(char *out, size_t out_size)
{
    ...
    ESP01_LOG_INFO("CIPSTATUS", "Statut TCP récupéré avec succès");
    return ESP01_OK;
}
...
ESP01_Status_t esp01_get_dhcp(bool *enabled)
{
    ...
    *enabled = (dhcp_mode & 1) != 0;
    ESP01_LOG_INFO("CWDHCP", "DHCP %s", *enabled ? "activé" : "désactivé");
    return ESP01_OK;
}
...
ESP01_Status_t esp01_disconnect_wifi(void)
{
    ...
    ESP01_LOG_INFO("CWQAP", "Déconnexion du WiFi...");
    ...
    ESP01_LOG_INFO("CWQAP", "Déconnecté avec succès");
    return ESP01_OK;
}

/**
 * @brief  Connecte au réseau WiFi.
 ...
 * 2. Configure le mode STA si nécessaire
 * 3. Établit la connexion avec commande AT+CWJAP
 * 4. Vérifie le succès de la connexion
 * 5. Récupère les informations IP
 */
ESP01_Status_t esp01_connect_wifi(const char *ssid, const char *password)
{
    ...
    // Vérification de la taille des chaînes pour éviter les débordements
    if (strlen(ssid) > 32 || strlen(password) > 64)
    {
        ESP01_LOG_ERROR("WIFI", "SSID ou mot de passe trop long");
    ...
    ESP01_LOG_INFO("WIFI", "Connexion au réseau %s...", ssid);
    ESP01_Status_t status = esp01_send_raw_command_dma(cmd, resp, sizeof(resp), "OK", ESP01_TIMEOUT_LONG);
    ...
    ESP01_LOG_INFO("WIFI", "Connexion réussie au réseau %s", ssid);
    return ESP01_OK;
} // Fin de esp01_connect_wifi

/**
 * @brief  Récupère l'adresse IP actuelle.
 * @param  ip_buf  Buffer pour stocker l'IP.
...
ESP01_Status_t esp01_get_current_ip(char *ip_buf, size_t buf_len)
{
    ...
    ESP01_LOG_INFO("CIFSR", "IP récupérée: %s", ip_buf);
    return ESP01_OK;
}
...
{
    VALIDATE_PARAM(channel, ESP01_INVALID_PARAM);
    ...
    if (st != ESP01_OK)
    {
        ESP01_LOG_ERROR("CHANNEL", "Erreur lors de la lecture du canal: %s", esp01_get_error_string(st));
        return st;
    }

    // Utilisation de la fonction de parsing pour extraire le canal
    if (esp01_parse_cwjap_response(resp, NULL, 0, NULL, 0, channel, NULL, NULL) != ESP01_OK)
    {
        ESP01_LOG_ERROR("CHANNEL", "Erreur de parsing de la réponse: %s", resp);
        return ESP01_FAIL;
    }

    ESP01_LOG_INFO("CHANNEL", "Canal WiFi: %d", *channel);
    return ESP01_OK;
}
    ...
    int *channel, int *rssi)
{
    ...
    ESP01_LOG_INFO("CONN_INFO", "Informations récupérées avec succès");
    return ESP01_OK;
}
    ...
    int *channel, int *rssi, int *enc_type)
{
    ...
    if (after_bssid)
    {
        ...
        if (enc_type && rssi_pos)
        {
            ...
            if (enc_pos && sscanf(enc_pos + 1, "%d", enc_type) != 1)
                return ESP01_PARSE_ERROR;
        }
    }
...
/**
 * @brief  Définit le nom d'hôte.
 * @param  hostname Nom d'hôte à configurer.
 * @return ESP01_Status_t
 */
ESP01_Status_t esp01_set_hostname(const char *hostname)
{
    ...
    ESP01_LOG_INFO("HOSTNAME", "Hostname configuré: %s", hostname);
    return ESP01_OK;
}

/**
 * @brief  Récupère le nom d'hôte.
 * @param  hostname Buffer pour stocker le nom d'hôte.
...
ESP01_Status_t esp01_get_hostname(char *hostname, size_t len)
{
    ...
    ESP01_LOG_INFO("HOSTNAME", "Hostname récupéré: %s", hostname);
    return ESP01_OK;
}
...
ESP01_Status_t esp01_scan_networks(esp01_network_t *networks, uint8_t max_networks, uint8_t *found_networks)
{
    ...
    ESP01_LOG_INFO("CWLAP", "Scan des réseaux WiFi...");
    ...
    *found_networks = 0;
    ...
    {
        // Cherche le début d'une entrée +CWLAP
        ...
        // Format: +CWLAP:(enc,"ssid",rssi,mac,ch,freq,reserved)
        if (sscanf(start, "+CWLAP:(%d,\"%[^\"]\",%d,", &enc, ssid, &rssi) != 3)
        {
            line = start + 1; // Avance pour chercher la prochaine entrée
            continue;
        }

        // Remplir la structure
        esp01_safe_strcpy(networks[*found_networks].ssid, ESP01_MAX_SSID_BUF, ssid);
        networks[*found_networks].rssi = rssi;

        // Convertir le code d'encryption en chaîne
        char enc_str[8];
        snprintf(enc_str, sizeof(enc_str), "%d", enc);
        esp01_safe_strcpy(networks[*found_networks].encryption, ESP01_MAX_ENCRYPTION_LEN, enc_str);

        (*found_networks)++;
        line = start + 1; // Avance pour chercher la prochaine entrée
    }

    ESP01_LOG_INFO("CWLAP", "%d réseaux trouvés", *found_networks);
    return ESP01_OK;
}
...
ESP01_Status_t esp01_get_wifi_connection(char *out, size_t out_size)
{
    ...
    ESP01_LOG_INFO("CWJAP?", "État de connexion WiFi récupéré");
    return ESP01_OK;
}
...
const char *esp01_connection_status_to_string(const char *resp)
{
    ...
    char ssid[ESP01_MAX_SSID_BUF] = {0};
    char bssid[ESP01_MAX_MAC_LEN] = {0};
    int channel = 0, rssi = 0;

    char *cwjap = strstr(resp, "+CWJAP:");
    if (!cwjap)
        return "Format non reconnu";

    if (sscanf(cwjap, "+CWJAP:\"%[^\"]\",\"%[^\"]\",%d,%d", ssid, bssid, &channel, &rssi) >= 3)
    {
    ...
    return "Format de connexion non reconnu";
}
...
ESP01_Status_t esp01_get_wifi_state(char *out, size_t out_size)
{
    ...
    ESP01_LOG_INFO("CWSTATE", "État WiFi récupéré");
    return ESP01_OK;
}
...
const char *esp01_cwstate_to_string(const char *resp)
{
    ...
    if (sscanf(cwstate, "+CWSTATE:%d,\"%[^\"]\"", &state, ssid) >= 1)
    {
 ...
 * @param  mac_buf  Buffer pour stocker l'adresse MAC.
 * @param  buf_len  Taille du buffer.
 * @return ESP01_Status_t
 */
ESP01_Status_t esp01_get_mac(char *mac_buf, size_t buf_len)
{
    ...
    ESP01_LOG_INFO("MAC", "Adresse MAC récupérée: %s", mac_buf);
    return ESP01_OK;
}
...
const char *esp01_rf_power_to_string(int rf_dbm)
{
    ...
    if (rf_dbm >= -30)
        sprintf(result, "%d dBm (Excellent)", rf_dbm);
    else if (rf_dbm >= -67)
        sprintf(result, "%d dBm (Très bon)", rf_dbm);
    else if (rf_dbm >= -70)
        sprintf(result, "%d dBm (Bon)", rf_dbm);
    else if (rf_dbm >= -80)
        sprintf(result, "%d dBm (Acceptable)", rf_dbm);
    else if (rf_dbm >= -90)
        sprintf(result, "%d dBm (Faible)", rf_dbm);
    else
        sprintf(result, "%d dBm (Très faible)", rf_dbm);
...
const char *esp01_tcp_status_to_string(const char *resp)
{
    ...
    int status_code = 0;
    if (sscanf(status_line, "STATUS:%d", &status_code) != 1)
        return "Format de statut invalide";

    char status_desc[64] = {0};
    switch (status_code)
    {
    case 0:
        return "WiFi non initialisé (0)";
    ...
    while (conn_line && conn_count < 10) // Limiter à 10 connexions max
    {
        conn_count++;
        int link_id;
        char type[16], remote_ip[32];
        int remote_port, local_port, tetype;

        if (sscanf(conn_line, "+CIPSTATUS:%d,\"%[^\"]\",\"%[^\"]\",%d,%d,%d",
                   &link_id, type, remote_ip, &remote_port, &local_port, &tetype) == 6)
        {
            char temp[128];
            ...
            if (strlen(conn_detail) + strlen(temp) < sizeof(conn_detail))
                strcat(conn_detail, temp);
            else
            {
                strcat(conn_detail, "\n  ... et plus");
                break;
...
const char *esp01_ping_result_to_string(const char *resp)
{
    ...
    char *ping_line = strstr(resp, "+PING:");
    if (!ping_line)
        return "Erreur: réponse ping non trouvée";

    int time_ms = 0;
    if (sscanf(ping_line, "+PING:%d", &time_ms) == 1)
    {
        snprintf(result, sizeof(result), "Ping réussi en %d ms", time_ms);
        return result;
    }

    return "Format de réponse ping non reconnu";
}
...
const char *esp01_cwqap_to_string(const char *resp)
{
    ...
    return "Déconnexion WiFi réussie";
}
...
ESP01_Status_t esp01_start_ap_config(const char *ssid, const char *password, int channel, int encryption)
{
    ...
    ESP01_LOG_INFO("CWSAP", "Configuration de l'AP \"%s\" sur le canal %d...", ssid, channel);
    ...
    ESP01_LOG_INFO("CWSAP", "AP configuré avec succès: SSID=\"%s\", Canal=%d, Encryption=%d (%s)",
...
ESP01_Status_t esp01_get_ap_config(char *out, size_t out_size)
{
    ...
    ESP01_LOG_INFO("CWSAP", "Configuration AP récupérée");
    return ESP01_OK;
}
...
const char *esp01_ap_config_to_string(const char *resp)
{
    ...
    char ssid[ESP01_MAX_SSID_BUF] = {0};
    char pwd[64] = {0};
    int channel = 1, encryption = 0, max_conn = 0, ssid_hidden = 0;

    char *cwsap = strstr(resp, "+CWSAP:");
    if (!cwsap)
        return "Format non reconnu";

    if (sscanf(cwsap, "+CWSAP:\"%[^\"]\",\"%[^\"]\",%d,%d,%d,%d",
               ssid, pwd, &channel, &encryption, &max_conn, &ssid_hidden) >= 4)
    {
        char enc_str[8];
        snprintf(enc_str, sizeof(enc_str), "%d", encryption);
        ...
        snprintf(result, sizeof(result),
                 ...
                 "  SSID caché: %s",
                 ssid, pwd, channel,
                 esp01_encryption_to_string(enc_str),
                 max_conn, ssid_hidden ? "Oui" : "Non");
        return result;
    }

    return "Format de configuration AP non reconnu";
}

/**
 * @brief  Récupère la force du signal WiFi (RSSI).
...
ESP01_Status_t esp01_get_rssi(int *rssi)
{
    ...
    if (*pos && sscanf(pos, "%d", rssi) == 1)
    {
        ESP01_LOG_INFO("RSSI", "Force du signal: %d dBm", *rssi);
        return ESP01_OK;
    }
...
/**
 * @brief  Convertit un code de statut TCP/IP en chaîne descriptive
 * @param  status_code  Code de statut (0-5)
 * @return Chaîne décrivant le statut
 */
const char *esp01_cipstatus_to_string(int status_code)
{
    switch (status_code)
    {
    case 0:
        return "Mode ESP-LINK (0)";
    case 1:
        return "Connexion TCP/UDP établie (1)";
    case 2:
        return "Station connectée au point d'accès (2)"; // Corrigé
    case 3:
        return "Transmission en cours (3)";
    case 4:
        return "Déconnexion en cours (4)";
    case 5:
        return "Connexion échouée (5)";
    default:
        return "Inconnu";
    }
}
...
/**
 * @brief  Effectue un ping vers une adresse IP ou un nom d'hôte.
 * @param  target Adresse IP ou nom d'hôte à pinguer.
 * @retval ESP01_Status_t
 */
ESP01_Status_t esp01_ping(const char *target)
{
    VALIDATE_PARAM(target, ESP01_INVALID_PARAM);

    char cmd[ESP01_SMALL_BUF_SIZE];
    char resp[ESP01_MAX_RESP_BUF];

    // Construction de la commande AT+PING
    snprintf(cmd, sizeof(cmd), "AT+PING=\"%s\"", target);
    ...
    const char *netmask)
{
    ...
    if (mode == ESP01_WIFI_MODE_AP) // Si le mode est AP (point d'accès)
    {
        ...
        status = esp01_send_raw_command_dma(cmd, resp, sizeof(resp), "OK", 2000); // Envoie la commande
    ...
    if (use_dhcp)
    {
        if (mode == ESP01_WIFI_MODE_STA)
        {
            ...
            status = esp01_send_raw_command_dma("AT+CWDHCP=1,1", resp, sizeof(resp), "OK", 2000); // Active le DHCP client
        }
        else if (mode == ESP01_WIFI_MODE_STA_AP)
        {
            ESP01_LOG_INFO("WIFI", "Activation du DHCP STA...");                                  // Log l'activation du DHCP STA
            status = esp01_send_raw_command_dma("AT+CWDHCP=1,1", resp, sizeof(resp), "OK", 2000); // Active le DHCP pour la station
        }
        else if (mode == ESP01_WIFI_MODE_AP)
        {
            ESP01_LOG_INFO("WIFI", "Activation du DHCP AP...");                                   // Log l'activation du DHCP pour l'AP
            status = esp01_send_raw_command_dma("AT+CWDHCP=2,1", resp, sizeof(resp), "OK", 2000); // Active le DHCP pour l'AP
        }
    ...
    else if (ip && gateway && netmask && mode == ESP01_WIFI_MODE_STA)
    {
        ...
        esp01_send_raw_command_dma("AT+CWQAP", resp, sizeof(resp), "OK", 2000); // Déconnecte le WiFi

        ESP01_LOG_INFO("WIFI", "Désactivation du DHCP client...");                            // Log la désactivation du DHCP client
        status = esp01_send_raw_command_dma("AT+CWDHCP=0,1", resp, sizeof(resp), "OK", 2000); // Désactive le DHCP client
    ...
    status = esp01_send_raw_command_dma("AT+CIPDINFO=1", resp, sizeof(resp), "OK", 2000);       // Active l'affichage IP client
...
/**
 * @brief  Récupère la configuration IP (statique) actuelle.
 * @param  ip_buf  Buffer pour l'adresse IP.
 * @param  ip_len  Longueur du buffer pour l'IP.
 * @param  gw_buf  Buffer pour la passerelle.
 * @param  gw_len  Longueur du buffer pour la passerelle.
 * @param  mask_buf Buffer pour le masque de sous-réseau.
 * @param  mask_len Longueur du buffer pour le masque.
 * @return ESP01_Status_t
 */
 ...
 * @param  channel Pointeur pour stocker le canal (NULL si non requis)
 * @retval ESP01_Status_t ESP01_OK en cas de succès ou code d'erreur
 * @note   Cette fonction envoie la commande AT+CWJAP? et parse la réponse
 *         Format de réponse attendu: +CWJAP:"SSID","xx:xx:xx:xx:xx:xx",channel,rssi
 * @details
 *         Le processus se déroule en trois étapes:
 *         1. Envoi de la commande AT+CWJAP?
 *         2. Réception et validation de la réponse
 *         3. Parsing de la réponse pour extraire les informations demandées
 *
 *         Si un paramètre est NULL, l'information correspondante n'est pas extraite.
 */
ESP01_Status_t esp01_get_connected_ap_info(char *ssid, char *bssid, uint8_t *channel)
{
    ...
    // Convertir uint8_t* en int* pour channel car la fonction esp01_parse_cwjap_response attend int*
    int ch_tmp = 0;
    ...
    // Convertir le résultat int en uint8_t si nécessaire
...
/**
 * @brief  Récupère le mode de connexion actuel.
 * @param  multi_conn Pointeur vers la variable de sortie.
 * @return ESP01_Status_t
 */
ESP01_Status_t esp01_get_connection_mode(uint8_t *multi_conn)
{
    ...
    ESP01_LOG_INFO("CIPMUX", "Récupération du mode de connexion...");
    ...
    *multi_conn = (uint8_t)mode;
    ESP01_LOG_INFO("CIPMUX", "Mode de connexion : %s", *multi_conn ? "Multi-connexion" : "Connexion unique");
    return ESP01_OK;
}
