...
// Fonction de test MQTT : connexion, publication, ping, déconnexion
void test_mqtt(void)
{
  ESP01_Status_t status;              // Statut de retour des fonctions ESP01
  char broker_ip[] = "192.168.1.185"; // Adresse IP du broker MQTT
  uint16_t broker_port = 1883;        // Port du broker MQTT

  esp01_flush_rx_buffer(1000); // Vide le buffer RX du module ESP01
  // esp01_mqtt_disconnect(); // <-- Laisser commenté pour éviter le double CIPCLOSE
  HAL_Delay(1000); // Attente pour stabilité
...
int main(void)
{
  ...
  printf("[TEST] === Test du scan WiFi ===\r\n");
  char wifi_list[1024];
  esp01_print_wifi_networks(wifi_list, sizeof(wifi_list));
  _esp_login("%s", wifi_list);
